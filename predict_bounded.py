...
# python predict_bounded.py --video footprints/monodepth2/ffff.avi --monodepth2_model_name mono+stereo_640x192 --pred_metric_depth
...
def test_simple(args,frame):
    ...
    with torch.no_grad():
            ...
            return opencvImage,metric_depth
...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            original_frame = cropped_frame(original_frame)
            ...
            for c in contours:
                ...
                #print(h)
                #print(depth_array)
                aaa = depth_array[(2*y + h)//2][(2*x + w)//2]
                cv2.putText(original_frame,"Object Detected range = {aa:.2f}".format(aa=aaa),(x+w+10,y+h),0,0.3,(0,255,0))
                if first:
                    out = cv2.VideoWriter('outpy2.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 20, (frame.shape[1],frame.shape[0]),1)
                    first=False
