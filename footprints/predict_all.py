...
class InferenceManager:
    ...
    def predict_hidden_depth(self, frame):
        ...
        """
        original, preprocessed_image = self._load_and_preprocess_image(frame)
...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        depth_array = test_simple(frame)
        #out.write(frame)
        background_image,_ = inference_manager._load_and_preprocess_image(self, depth_array)
        visualisation = background_image * (1 - hidden_ground) + depth_colourmap * hidden_ground
        frame = (visualisation[:, :, ::-1] * 255).astype(np.uint8)
